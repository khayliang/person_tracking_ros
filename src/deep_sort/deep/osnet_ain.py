...
def init_pretrained_weights(model, checkpoint, key=''):
...
##########
def osnet_ain_x1_0(
    checkpoint, num_classes=1000, pretrained=True, loss='softmax', **kwargs
):
    model = OSNet(
        num_classes,
        blocks=[
    ...
    if pretrained:
        init_pretrained_weights(model, key='osnet_ain_x1_0', checkpoint=checkpoint)
    model.eval()
    return model